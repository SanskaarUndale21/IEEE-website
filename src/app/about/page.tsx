...
export default function AboutPage() {
  ...
  return (
    <main className="bg-[var(--bg)]">
      ...
      {/* ── Hero ──────────────────────────────────────────────── */}
      <section className="relative overflow-hidden px-5 pt-20 pb-16 md:px-8 md:pt-24 md:pb-20">
        {/* Background image */}
        <div className="pointer-events-none absolute inset-0">
          <Image src={IMAGES.event1} alt="IEEE SGBIT" fill className="object-cover opacity-8 dark:opacity-12" />
          <div className="absolute inset-0 bg-gradient-to-b from-[var(--bg)]/60 via-[var(--bg)]/70 to-[var(--bg)]" />
        </div>

        {/* Ambient glow */}
        <div className="pointer-events-none absolute left-1/2 top-1/2 h-[500px] w-[500px] -translate-x-1/2 -translate-y-1/2 rounded-full bg-ieee-blue/[0.05] blur-[180px]" />
        ...
        <div className="relative z-10 mx-auto max-w-4xl text-center">
          ...
          >
            <Link
              href="/"
              className="mb-6 inline-flex items-center gap-2 text-[11px] font-bold uppercase tracking-[0.2em] text-[var(--text-muted)] transition-colors hover:text-[var(--ieee-blue)] dark:hover:text-[var(--ieee-light)]"
            >
            ...
            <p className="section-label mb-3 mt-4">About Us</p>
            <h1 className="font-display text-4xl font-bold text-gray-900 dark:text-white md:text-5xl lg:text-6xl">
              IEEE SGBIT<br />
              <span className="gradient-text">Student Branch</span>
            </h1>
            <p className="mx-auto mt-5 max-w-2xl text-sm leading-relaxed text-[var(--text-secondary)] md:text-base">
          ...
          >
            {ABOUT_STATS.map((stat, i) => (
              <motion.div
                ...
                className="group relative overflow-hidden rounded-xl border border-[var(--border)] bg-[var(--bg-card)] p-5 transition-all duration-300 hover:-translate-y-0.5 hover:border-[var(--ieee-blue)]/30 hover:shadow-[0_8px_24px_rgba(0,98,155,0.12)]"
              >
                ...
                <p className="font-display text-3xl font-black text-[var(--ieee-blue)] dark:text-[var(--ieee-light)]">{stat.value}</p>
                <p className="mt-1 text-[11px] font-medium text-[var(--text-muted)]">{stat.label}</p>
              </motion.div>
            ))}
          </motion.div>
        </div>
      ...
      <section ref={missionRef} className="border-t border-[var(--border)] px-5 py-16 md:px-8 md:py-24">
        <div className="mx-auto max-w-7xl grid gap-14 lg:grid-cols-2 lg:items-center">
          ...
          >
            {[IMAGES.event1, IMAGES.collegeTopView, IMAGES.excom, IMAGES.event1].map((src, i) => (
