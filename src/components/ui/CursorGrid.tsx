...
/**
 * CursorGrid — canvas lattice whose cells light up around the pointer and
 * ripple outwards on click. Ported to TS from the React Bits JS + CSS variant.
 *
 * Two changes over the original: the pointer is tracked on `window` (so the
 * grid can sit behind content and still react), and the loop parks itself
...
export default function CursorGrid({
  ...
  useEffect(() => {
    ...
    const draw = (now: number) => {
      ...
      if (anyVisible && onScreen && !document.hidden) {
    ...
    // window-level tracking: the grid renders behind content, so it never
    // receives pointer events of its own
    const onPointerMove = (e: PointerEvent) => {
      const rect = canvas.getBoundingClientRect();
      const x = e.clientX - rect.left;
      const y = e.clientY - rect.top;
      if (x < -radius || y < -radius || x > rect.width + radius || y > rect.height + radius) return;
      energize(x, y);
      wake();
    };
