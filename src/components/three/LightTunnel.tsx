...
/**
 * LightTunnel — fibre-optic light tunnel rendered on a single full-screen
 * triangle with OGL. Ported to TS from the React Bits JS + CSS variant, with
 * the stylesheet folded into the component and a WebGL2 guard added (the
 * shaders are `#version 300 es`, so a WebGL1 fallback context must bail out).
 */
...
type TunnelCtx = { program: Program };
...
export default function LightTunnel({
  ...
  useEffect(() => {
    ...
    try {
      renderer = new Renderer({
        ...
        antialias: false,
        dpr: Math.min(window.devicePixelRatio || 1, dprRef.current),
      });
    } catch {
      return; // no WebGL at all — the layer simply stays empty
    }

    const gl = renderer.gl;
    // the shaders are GLSL ES 3.00; a WebGL1 fallback context cannot compile them
    ...
    const program = new Program(gl, {
      ...
      uniforms: {
        ...
        uOpacity: { value: 1.0 },
        uCableColor: { value: new Float32Array([0.658, 0.333, 0.968]) },
        uPulseColor: { value: new Float32Array([0.658, 0.333, 0.968]) },
        uTunnelColor: { value: new Float32Array([0.321, 0.152, 1]) },
    ...
    // tracked on the window, not the canvas: the tunnel sits behind the hero
    // content, so pointer events never reach it
    ...
    const loop = (t: number) => {
      ...
      const tx = mouseEnabledRef.current ? targetMouse[0] : 0.5;
      const ty = mouseEnabledRef.current ? targetMouse[1] : 0.5;
  ...
  useEffect(() => {
    ...
    write(u.uCableColor.value as Float32Array, cableColor);
    write(u.uPulseColor.value as Float32Array, pulseColor);
    write(u.uTunnelColor.value as Float32Array, tunnelColor);
