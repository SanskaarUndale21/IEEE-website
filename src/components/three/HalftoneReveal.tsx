...
/**
 * HalftoneReveal — renders an image as a halftone print that sharpens into
 * a photographic "loupe" around the cursor. Ported to TS from the React
 * Bits JS + CSS variant, with the stylesheet folded into inline canvas
 * styles + a tailwind wrapper class (matching this repo's LightTunnel port).
 */
...
export default function HalftoneReveal({
  ...
  return (
    <div
      ref={containerRef}
      className={`relative h-full w-full touch-none overflow-hidden [cursor:crosshair] ${className}`.trim()}
      style={{ borderRadius }}
    />
  );
}
